...
| Not explicitly setting input with `@observe()` | All function args become trace input (including API keys, configs) | Python: use `langfuse_context.update_current_span(input=...)`. JS/TS: use `updateActiveObservation({ input: ... })`. Set only the relevant input (e.g., user message) |
